...
function Library:_IsThemeIgnored(object)
    ...
    while current and current ~= self.ScreenGui do
        if current:GetAttribute("NexzanIgnoreTheme") == true then return true end
...
function Library:_BindThemeProperty(object, propertyName)
    ...
    local attributeName = "NexzanTheme_" .. propertyName
...
function Library:SetTheme(themeName)
    if type(themeName) ~= "string" or not self.ThemePresets[themeName] then
        warn("[Nexzan Hub] Theme tidak ditemukan: " .. tostring(themeName))
        return false
    end
...
-- ==================== KEY SYSTEM (GAYA WINDUI) ====================
-- Dapat dikonfigurasi dari script pemanggil melalui Library.new({KeySystem={...}})
-- atau dipasang sesudah Window dibuat dengan Window:CreateKeySystem({...}).
function Library:_ValidateConfiguredKey(config, enteredKey)
    local rawKey = tostring(enteredKey or "")
    local key = config.TrimKey == false and rawKey or rawKey:match("^%s*(.-)%s*$")

    if type(config.Verify) == "function" then
        local ok, valid, message = pcall(config.Verify, key, self)
        if not ok then
            return false, "Validator error: " .. tostring(valid)
        end
        return valid == true, message or (valid and "Key valid." or "Key tidak valid.")
    end

    local configuredKeys = config.Key or config.Keys
    if type(configuredKeys) == "function" then
        local ok, result = pcall(configuredKeys, self)
        if ok then configuredKeys = result else return false, "Gagal mengambil daftar Key." end
    end
    if type(configuredKeys) ~= "table" then configuredKeys = {configuredKeys} end

    local caseSensitive = config.CaseSensitive ~= false
    local comparisonKey = caseSensitive and key or string.lower(key)
    for _, validKey in ipairs(configuredKeys) do
        if validKey ~= nil then
            local candidate = tostring(validKey)
            candidate = config.TrimKey == false and candidate or candidate:match("^%s*(.-)%s*$")
            candidate = caseSensitive and candidate or string.lower(candidate)
            if comparisonKey == candidate then
                return true, config.SuccessMessage or "Key berhasil diverifikasi."
            end
        end
    end
    return false, config.InvalidMessage or "Key salah atau sudah tidak berlaku."
end

function Library:CreateKeySystem(config)
    config = type(config) == "table" and config or {}

    if self.KeySystem and self.KeySystem.Destroy then
        self.KeySystem:Destroy()
    end

    self.KeyUnlocked = false
    local attempts = 0
    local validating = false
    local destroyed = false
    local maxAttempts = math.max(0, tonumber(config.MaxAttempts) or 0)
    local lockTime = math.max(1, tonumber(config.LockTime) or 15)
    local saveFile = tostring(config.FileName or config.SaveFile or "NexzanHub_Key.txt")
    saveFile = saveFile:gsub("[^%w%._%-/]", "_")

    local overlay = Instance.new("Frame")
    overlay.Name = "NexzanKeySystem"
    ...
    card.Name = "KeyCard"
    card.Size = UDim2.new(0, 320, 0, 232)
    card.AnchorPoint = Vector2.new(0.5, 0.5)
    card.Position = UDim2.new(0.5, 0, 0.5, 0)
    card.BackgroundColor3 = self.Theme.MainBackground
    ...
    cardStroke.Color = self.Theme.MainBorder
    ...
    accentLine.Position = UDim2.new(0, 7, 0, 10)
    ...
    local thumbnailConfig = type(config.Thumbnail) == "table" and config.Thumbnail or nil
    local thumbnail
    if thumbnailConfig and thumbnailConfig.Image and tostring(thumbnailConfig.Image) ~= "" then
        thumbnail = Instance.new("ImageLabel")
        thumbnail.Name = "Thumbnail"
        thumbnail.Size = UDim2.new(0, 42, 0, 42)
        thumbnail.Position = UDim2.new(1, -74, 0, 12)
        thumbnail.BackgroundColor3 = self.Theme.Input
        thumbnail.BackgroundTransparency = 0.1
        thumbnail.BorderSizePixel = 0
        thumbnail.Image = tostring(thumbnailConfig.Image)
        thumbnail.ScaleType = Enum.ScaleType.Crop
        thumbnail.ZIndex = 102
        thumbnail.Parent = card
        local thumbCorner = Instance.new("UICorner")
        thumbCorner.CornerRadius = UDim.new(0, 7)
        thumbCorner.Parent = thumbnail
        local thumbStroke = Instance.new("UIStroke")
        thumbStroke.Color = self.Theme.MainBorder
    ...
    titleLabel.Size = UDim2.new(1, thumbnail and -105 or -42, 0, 22)
    titleLabel.Position = UDim2.new(0, 20, 0, 12)
    titleLabel.BackgroundTransparency = 1
    titleLabel.Font = Enum.Font.SourceSansBold
    titleLabel.Text = tostring(config.Title or "Key System")
    ...
    noteLabel.Name = "Note"
    noteLabel.Size = UDim2.new(1, thumbnail and -105 or -40, 0, 38)
    noteLabel.Position = UDim2.new(0, 20, 0, 35)
    noteLabel.BackgroundTransparency = 1
    noteLabel.Font = Enum.Font.SourceSans
    noteLabel.Text = tostring(config.Note or "Masukkan Key untuk membuka UI.")
    noteLabel.TextColor3 = self.Theme.TextSecondary
    ...
    closeKeyButton.Name = "Close"
    closeKeyButton.Size = UDim2.new(0, 20, 0, 20)
    closeKeyButton.Position = UDim2.new(1, -27, 0, 7)
    closeKeyButton.BackgroundColor3 = self.Theme.Control
    closeKeyButton.BackgroundTransparency = 0.25
    closeKeyButton.Text = "×"
    closeKeyButton.TextColor3 = self.Theme.TextPrimary
    closeKeyButton.TextSize = 15
    closeKeyButton.Font = Enum.Font.SourceSansBold
    closeKeyButton.Visible = config.CanClose ~= false
    closeKeyButton.ZIndex = 104
    closeKeyButton.Parent = card
    ...
    inputHolder.Position = UDim2.new(0, 20, 0, 79)
    inputHolder.BackgroundColor3 = self.Theme.Input
    inputHolder.BackgroundTransparency = 0.05
    inputHolder.BorderSizePixel = 0
    inputHolder.ZIndex = 102
    inputHolder.Parent = card
    local inputCorner = Instance.new("UICorner")
    inputCorner.CornerRadius = UDim.new(0, 6)
    inputCorner.Parent = inputHolder
    local inputStroke = Instance.new("UIStroke")
    inputStroke.Color = self.Theme.MainBorder
    inputStroke.Thickness = 1
    inputStroke.Parent = inputHolder

    local keyBox = Instance.new("TextBox")
    keyBox.Name = "KeyInput"
    keyBox.Size = UDim2.new(1, -14, 1, 0)
    keyBox.Position = UDim2.new(0, 7, 0, 0)
    keyBox.BackgroundTransparency = 1
    keyBox.ClearTextOnFocus = false
    keyBox.Font = Enum.Font.SourceSans
    keyBox.PlaceholderText = tostring(config.Placeholder or "Masukkan Key...")
    keyBox.PlaceholderColor3 = self.Theme.Placeholder
    keyBox.Text = tostring(config.DefaultKey or "")
    ...
    local getKeySource = config.URL or config.Link or config.GetKey
    local hasGetKey = getKeySource ~= nil and getKeySource ~= ""

    local getKeyButton = Instance.new("TextButton")
    getKeyButton.Name = "GetKey"
    getKeyButton.Size = hasGetKey and UDim2.new(0.44, -3, 0, 30) or UDim2.new(0, 0, 0, 30)
    getKeyButton.Position = UDim2.new(0, 20, 0, 122)
    getKeyButton.BackgroundColor3 = self.Theme.Control
    getKeyButton.BackgroundTransparency = 0.05
    getKeyButton.Text = tostring(config.GetKeyText or "Get Key")
    getKeyButton.TextColor3 = self.Theme.TextPrimary
    getKeyButton.TextSize = 11
    getKeyButton.Font = Enum.Font.SourceSansBold
    getKeyButton.Visible = hasGetKey
    ...
    getKeyStroke.Color = self.Theme.MainBorder
    getKeyStroke.Thickness = 1
    getKeyStroke.Parent = getKeyButton

    local verifyButton = Instance.new("TextButton")
    verifyButton.Name = "Verify"
    verifyButton.Size = hasGetKey and UDim2.new(0.56, -3, 0, 30) or UDim2.new(1, -40, 0, 30)
    verifyButton.Position = hasGetKey and UDim2.new(0.44, 23, 0, 122) or UDim2.new(0, 20, 0, 122)
    verifyButton.BackgroundColor3 = self.Theme.Accent
    verifyButton.BackgroundTransparency = 0.12
    verifyButton.Text = tostring(config.VerifyText or "Verify Key")
    verifyButton.TextColor3 = self.Theme.White
    verifyButton.TextSize = 11
    verifyButton.Font = Enum.Font.SourceSansBold
    verifyButton.ZIndex = 102
    verifyButton.Parent = card
    local verifyCorner = Instance.new("UICorner")
    verifyCorner.CornerRadius = UDim.new(0, 6)
    verifyCorner.Parent = verifyButton
    local verifyStroke = Instance.new("UIStroke")
    verifyStroke.Color = self.Theme.Accent
    ...
    statusLabel.Position = UDim2.new(0, 20, 0, 159)
    statusLabel.BackgroundColor3 = self.Theme.Element
    statusLabel.BackgroundTransparency = 0.45
    statusLabel.BorderSizePixel = 0
    statusLabel.Font = Enum.Font.SourceSans
    statusLabel.Text = tostring(config.StatusText or "Silakan masukkan Key yang valid.")
    ...
    footerLabel.Position = UDim2.new(0, 20, 1, -28)
    footerLabel.BackgroundTransparency = 1
    footerLabel.Font = Enum.Font.SourceSansItalic
    footerLabel.Text = config.SaveKey == true and "Key akan disimpan otomatis setelah valid." or tostring(config.Footer or "Nexzan Hub Key System")
    footerLabel.TextColor3 = self.Theme.TextSecondary
    ...
    local function setStatus(text, color)
        ...
        statusLabel.TextColor3 = color or self.Theme.TextSecondary
    end

    local function saveKey(key)
        if config.SaveKey ~= true then return end
        pcall(function()
            if writefile then writefile(saveFile, tostring(key)) end
        end)
    end

    local function deleteSavedKey()
        pcall(function()
            if delfile and isfile and isfile(saveFile) then delfile(saveFile) end
        end)
    end
    ...
    local function unlock(key, message)
        ...
        saveKey(key)
        setStatus(message or config.SuccessMessage or "Key valid!", Color3.fromRGB(70, 220, 120))
        playSound(Sounds.Notification, 0.35)
        if type(config.OnSuccess) == "function" then
            task.spawn(function() pcall(config.OnSuccess, key, self) end)
        end
        task.delay(tonumber(config.SuccessDelay) or 0.35, function()
            if not destroyed and overlay.Parent then
    ...
    function KeyAPI:Validate(key, silent)
        if destroyed or validating then return false end
        validating = true
        verifyButton.Text = tostring(config.ValidatingText or "Checking...")
        verifyButton.AutoButtonColor = false
        local entered = tostring(key ~= nil and key or keyBox.Text)
        local valid, message = self._owner:_ValidateConfiguredKey(config, entered)
        validating = false
        verifyButton.Text = tostring(config.VerifyText or "Verify Key")
        verifyButton.AutoButtonColor = true

        if valid then
            unlock(entered, message)
            return true
        end

        attempts += 1
        if not silent then
            setStatus(message or config.InvalidMessage or "Key tidak valid.", Color3.fromRGB(255, 95, 95))
            playSound(Sounds.ToggleOff, 0.3)
        end
        deleteSavedKey()
        if type(config.OnFail) == "function" then
            task.spawn(function() pcall(config.OnFail, entered, attempts, message, self._owner) end)
        end
        ...
        if maxAttempts > 0 and attempts >= maxAttempts then
            ...
            for remaining = lockTime, 1, -1 do
                if destroyed then break end
                setStatus("Terlalu banyak percobaan. Tunggu " .. remaining .. " detik.", Color3.fromRGB(255, 180, 70))
                task.wait(1)
            end
            if not destroyed then
                verifyButton.Active = true
                keyBox.TextEditable = true
                setStatus(config.StatusText or "Silakan coba kembali.", self._owner.Theme.TextSecondary)
            end
        end
        return false
    end

    function KeyAPI:SetKey(key)
        keyBox.Text = tostring(key or "")
        return self
    end

    function KeyAPI:GetEnteredKey()
        return keyBox.Text
    end

    function KeyAPI:GetLink()
        if type(getKeySource) == "function" then
            local ok, result = pcall(getKeySource, self._owner)
            return ok and result or nil
        end
        return getKeySource
    end

    function KeyAPI:Open()
        if not destroyed then overlay.Visible = true end
        return self
    end

    function KeyAPI:Close()
        if not destroyed then overlay.Visible = false end
        return self
    end

    function KeyAPI:IsUnlocked()
        return self._owner.KeyUnlocked == true
    end

    function KeyAPI:ResetSavedKey()
        deleteSavedKey()
        return self
    end

    function KeyAPI:Destroy()
        if destroyed then return end
        destroyed = true
        if overlay then overlay:Destroy() end
        if self._owner.KeySystem == self then self._owner.KeySystem = nil end
    end

    KeyAPI._owner = self
    KeyAPI.Instance = overlay
    KeyAPI.Input = keyBox
    self.KeySystem = KeyAPI

    verifyButton.MouseButton1Click:Connect(function()
        KeyAPI:Validate(keyBox.Text)
    end)
    keyBox.FocusLost:Connect(function(enterPressed)
        if enterPressed then KeyAPI:Validate(keyBox.Text) end
    end)
    getKeyButton.MouseButton1Click:Connect(function()
        local link = KeyAPI:GetLink()
        if link and tostring(link) ~= "" then
            local copied = pcall(function()
                local clipboard = setclipboard or toclipboard
                if not clipboard then error("Clipboard tidak didukung") end
                clipboard(tostring(link))
            end)
            if copied then
                setStatus(config.CopiedMessage or "Link Get Key berhasil disalin.", Color3.fromRGB(90, 190, 255))
            else
                setStatus("Tidak dapat menyalin link pada executor ini.", Color3.fromRGB(255, 180, 70))
            end
        end
    end)
    closeKeyButton.MouseButton1Click:Connect(function()
        if type(config.OnClose) == "function" then pcall(config.OnClose, self) end
    ...
    makeDraggable(card, titleLabel)

    -- Muat dan validasi Key tersimpan secara otomatis jika fitur SaveKey aktif.
    if config.SaveKey == true then
        task.spawn(function()
            local saved
            pcall(function()
                if isfile and readfile and isfile(saveFile) then saved = readfile(saveFile) end
            end)
            if saved and saved ~= "" and not destroyed then
                keyBox.Text = saved
                setStatus("Memeriksa Key tersimpan...", self.Theme.TextSecondary)
                task.wait(0.1)
                KeyAPI:Validate(saved, false)
            end
        end)
    end
...
Library.KeySystem = Library.CreateKeySystem

function Library:IsKeyUnlocked()
    return self.KeyUnlocked == true
end
...
function Library.new(options)
    ...
    local requestedTheme = type(options) == "string" and options
        ...
        or "Default"
    local requestedKeySystem = type(options) == "table" and options.KeySystem or nil
    self.ThemePresets = {}
    ...
    -- KOTAK INFO PENGGUNA (KIRI): avatar, DisplayName, username asli, dan logo NH
    ...
    createNHLogo(logoHolder, UDim2.new(1, 0, 1, 0), 8)
    ...
    -- Key System bersifat opsional dan seluruh konfigurasinya berasal dari script pemanggil.
    if type(requestedKeySystem) == "table" then
        self:CreateKeySystem(requestedKeySystem)
    else
        self.KeyUnlocked = true
    end
...
-- local tag = Window:Tag({ Title = "v1.6.6", Icon = "rbxassetid://ID", Color = Color3.fromRGB(48, 255, 106), Radius = 6 })
function Library:Tag(options)
    ...
    tag:SetAttribute("NexzanIgnoreTheme", true) -- Warna Tag tetap dikontrol melalui Tag:SetColor().
    ...
    local function normalizeIcon(value)
        ...
        if string.find(raw, "rbxasset", 1, true) then return raw end
        local numericId = string.match(raw, "%d+")
        return numericId and ("rbxassetid://" .. numericId) or ""
    end
